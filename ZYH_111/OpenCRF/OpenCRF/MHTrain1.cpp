...
void CRFModel::MHTrain1(Config *conf)
{
	...
	for (int iter = 0; iter < max_iter; iter++)
	{
		...
		double norm = 0;
        norm = double(batch_size); // / double(N);
        for (int pid = 0; pid < num_feature; pid++) {
            // double s1 = 0, s2 = 0; 
            // for (int thread_id = 0; thread_id < num_thread; thread_id++) {
            //     s1 += sum1[thread_id][pid]; sum1[thread_id][pid] = 0;
            //     s2 += sum2[thread_id][pid]; sum2[thread_id][pid] = 0;
            // }
...
bool CRFModel::train1_sample(int type, int center, int ynew, double p, vector<int>& _state, map<int,double>& _gradient)
{
	...
	int y_center;
    //if (type == 1 && label_type[center] == KNOWN_LABEL) {
    //  return;
    //}
    ...
    // change Y to Ynew
    _state[center] = ynew; //rand() % num_class;//sample(center, _state); //
    // calculate for Ynew
	for (int j = 0; j < sample->node[u]->num_attrib; j++) {
    //for (int j = 0; j < feature_id[u].size(); j++) {
        //int pid = get_node_param_id(_state[u], feature_id[u][j]);
